...
STOCK_INFO = StockInfo(
    code="2330", price=2395, market_cap_millions=62108026, beta=1.10,
    pe_ratio=27.76, dividend_yield_pct=0.92, book_value_per_share=248.05,
...
def _patch_all_sources(**overrides):
    defaults = {
        ...
        "app.ingest.fetch_margin_quarters": lambda code, client: [
            MarginQuarter(
                quarter="115.2Q", revenue=2404483690, cost_of_goods_sold=792877574,
                gross_profit=1611606116, gross_margin_pct=67.03, operating_income=1425568793,
                operating_margin_pct=59.29, non_operating_income=124660980,
                pretax_income=1550229773, net_income=1279582227, eps=49.33,
            )
        ],
        "app.ingest.fetch_quarterly_turnover": lambda code, client: [
            QuarterlyTurnover(quarter="2026Q2", ar_days=29, inventory_days=87, operating_cycle_days=116)
        ],
        "app.ingest.fetch_quarterly_eps": lambda code, client: [QuarterlyEps(quarter="2026Q2", eps=27.25)],
        "app.ingest.fetch_financial_health": lambda code, client: [
            FinancialHealthQuarter(
                code="2330", quarter="2026Q2", current_assets=4623887000, total_assets=9375654727,
                current_liabilities=1857761825, total_liabilities=2901183746, total_equity=6474470981,
                capital=259323701, book_value_per_share=248.05, revenue=2404483690,
                gross_profit=1611606116, operating_income=1425568793, pretax_income=1550229773,
                net_income=1279582227, eps=49.33, gross_margin_pct=67.03, operating_margin_pct=59.29,
                net_margin_pct=53.22,
            )
        ],
        "app.ingest.fetch_dividend_history": lambda code, client: [
            DividendEvent(
                fiscal_year=2025, payout_year=2026, ex_dividend_date="06/11", pre_price=2255.0,
                stock_dividend=0.0, cash_dividend=6.0, eps=66.26, payout_ratio_pct=9.06, cash_yield_pct=0.27,
            )
        ],
        "app.ingest.fetch_quarterly_cashflow": lambda code, client: [
            QuarterlyCashflow(
                quarter="2026Q2", operating=783360, investing=-497530, financing=-187250,
                free_cash_flow=287360,
            )
        ],
        "app.ingest.fetch_daily_chips": lambda code, client: [
            DailyChips(
                date="2026-08-14", concentration_pct=75.2, foreign_holding_pct=71.5,
...
def test_refresh_stock_populates_every_table(tmp_path):
    ...
    patches = [patch(target, side_effect=fn) for target, fn in _patch_all_sources().items()]
    for p in patches:
        p.start()
    try:
    ...
    assert conn.execute("SELECT COUNT(*) c FROM financial_health_quarterly").fetchone()["c"] == 1
    assert conn.execute("SELECT COUNT(*) c FROM dividends").fetchone()["c"] == 1
    assert conn.execute("SELECT COUNT(*) c FROM cashflow_quarterly").fetchone()["c"] == 1
